...
int main()
{
    ...
    const std::regex helpCommand("^((H|h)elp|HELP)$");
    const std::regex listCommand("^((L|l)ist|LIST)$");
    const std::regex exitCommand("^((E|e)xit|EXIT)$");
    const std::regex buyCommand("^([A-Za-z]+\\s[0-9]+(\\s)*)+$");
    ...
    while (inProgram)
    {
        ...
        else if (std::regex_match(userInput, buyCommand))
        {
            ...
            const std::regex itemQuantityExp("[A-Za-z]+\\s[0-9]+");
            const std::regex itemExp("[A-Za-z]+");
            const std::regex quantityExp("[0-9]+");
